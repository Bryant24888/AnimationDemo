...
interface ControlsProps {
  ...
  isPaused: boolean;
  isPlaying: boolean;
  canTriggerResult: boolean;
}

const Button: React.FC<React.PropsWithChildren<{ onClick: () => void; className?: string, disabled?: boolean }>> = ({ onClick, children, className, disabled }) => (
  <button
    onClick={onClick}
    disabled={disabled}
    className={`flex items-center gap-2 bg-slate-800 border border-slate-700 text-slate-200 rounded-lg px-4 py-2 transition-all duration-200 ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-slate-500 hover:bg-slate-700'} ${className}`}
  >
    {children}
  </button>
);
...
const Controls: React.FC<ControlsProps> = ({ onPlay, onPause, onSuccess, onFail, isPaused, isPlaying, canTriggerResult }) => {
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <Button onClick={onSuccess} disabled={!canTriggerResult} className="text-green-400">
      ...
      <Button onClick={onFail} disabled={!canTriggerResult} className="text-red-400">
